document.addEventListener('DOMContentLoaded', () => {
  let vendor = (typeof getCurrentUserOfRole === 'function' ? getCurrentUserOfRole('vendor') : null) || getCurrentUser();
  let token = (typeof getAuthTokenForRole === 'function' ? getAuthTokenForRole('vendor') : null) || getAuthToken();
  ...
  // Bootstrap: ensure vendor profile is ready, then render categories and load data
  (async function init() {
    try {
      if (preparingEl) preparingEl.classList.remove('hidden');
      if (submitBtnGlobal) submitBtnGlobal.disabled = true;

      await ensureVendorProfileReady();
      ...
      if (categorySelect) {
        ...
        if (categories.length) {
          ...
          // Hide select to avoid iOS count rendering
  ...
  async function ensureVendorProfileReady() {
    // Try to fetch fresh vendor profile; refresh token once on 401
    try {
      const r = await fetch(`${BACKEND}/api/vendors/profile?force=1`, {
      ...
      if (r.status === 401) {
        ...
        if (rr.ok) {
          ...
          if (data && data.token) {
            ...
            token = data.token;
            const r2 = await fetch(`${BACKEND}/api/vendors/profile?force=1`, {
              headers: { Authorization: `Bearer ${token}` }
            });
            if (r2.ok) {
              ...
              if (prof && prof.vendor) {
                vendor = prof.vendor;
                localStorage.setItem('vendplugVendor', JSON.stringify(vendor));
                return;
              }
            }
          }
      ...
      // If all fails, continue with cached vendor
    } catch (_) {
      // Network issue: continue with cached vendor
    }
  }
  ...
  async function submitProduct() {
    ...
    // Determine categories from select (supports single or multiple); fallback to vendor profile
    ...
    if (!selectedCategories.length) {
      const fromProfile = Array.isArray(vendor?.category) ? vendor.category : (vendor?.category ? [vendor.category] : []);
